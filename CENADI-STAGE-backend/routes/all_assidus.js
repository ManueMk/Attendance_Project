...
router.post('/all_assidus',authMiddleware, (req, res, next) => {
          ...
          if (result.rows.length > 0) {
            ...
            db.query(
              ...
              FROM (
                ...
                AND EXTRACT(DOW FROM $1::date + INTERVAL '1 day' * n) IN (6, 7)
              ) d`,
              [dateA, dateD], 
              (err, result) => {
                if (err) {
                ...
                if (result.rows ) {
                 ...
                 // return res.status(200).json({ nombreDeJours: dateD, dateA,nombreDeWeekEnd})
                      ...
                      if (result.rows[0]) {
                        ...
                        db.query(
                          ...
                          (err, result) => {
                            ...
                            // return res.status(200).json({ nombreDeJours: nombreDeSecondes })
                            ...
                            if (result.rows[0]) {
                              ...
                              const nombreDePresenceEffectivePersonnel = nombreDeSecondes;
                              //le pourcentage d'assiduite de chaque employes
                             db.query(
  `SELECT presences."matricule", agt."nom", agt."prenom", agt."division",
   CASE WHEN SUM(EXTRACT(EPOCH FROM ("presences"."depart" - "presences"."arrive"))::BIGINT - 28800) <= 0 THEN '00:00:00' ELSE TO_CHAR(make_interval
    (secs => SUM(EXTRACT(EPOCH FROM ("presences"."depart" - "presences"."arrive"))::BIGINT - 28800)), 'HH24:MI:SS') END AS "heureSupp",
     TO_CHAR(make_interval(secs => SUM(EXTRACT(EPOCH FROM ("presences"."depart" - "presences"."arrive"))::BIGINT)), 'HH24:MI:SS') AS "tempsEff", 
     (SUM(EXTRACT(EPOCH FROM ("presences"."depart" - "presences"."arrive"))::BIGINT) / $3)* 
     100 AS "pourcentage" FROM "agt" INNER JOIN "presences" ON "presences"."matricule" = "agt"."matricule" WHERE "presences"."date" BETWEEN $1::date AND $2::date AND "presences"."isdeleted" =0 
     GROUP BY "presences"."matricule", "agt"."nom", "agt"."prenom", agt."division" ORDER BY "pourcentage"  DESC`,
                                ...
                                (err, result) => {
                                  ...
                                  if (result.rows) {
                                    return res
                                      .status(200)
                                      .json({ le_personnel_le_plus_assidu: result.rows});
                                  } else {
                                    return res
                                      .status(500)
                                      .json({
                                        erreur:
                                          "une erreur est survenue sur le calcul des heures_sup personnelles",
                                      });
                                  }
                                }
