...
router.post("/chef_div", authMiddleware, (req, res, next) => {
  ...
  db.query(
    `SELECT COUNT(*) AS nombreEmploye FROM "agt" WHERE "chef" = $1`,
    [1],
    (err, result) => {
      if (err) {
        throw err;
      ...
      WHERE "agt"."chef" = $1 AND "presences"."date" BETWEEN $2 AND $3`,
          [1, dateA, dateD],
          (err, result) => {
            if (err) {
              throw err;
          ...
          FROM (
            ...
            AND EXTRACT(DOW FROM $1::date + INTERVAL '1 day' * n) IN (5, 6)
                ...
                (err, result) => {
                  ...
                  if (result.rows[0]) {
                    ...
                    db.query(
                      ...
                      (err, result) => {
                        ...
                        const nombreDeJours =
                          result.rows[0].nombredejours - nombreDeWeekEnd + 1;
                        ...
                        if (result.rows[0]) {
                          ...
                          const nombrePresenceNormal =
                            nombreEmployes * nombreDeSecondes; //pour tout les employés

                          db.query(
                            `SELECT SUM(EXTRACT(EPOCH FROM ("depart" - "arrive"))) AS temps
                    FROM "presences"
                    INNER JOIN agt ON "presences"."matricule" = "agt"."matricule"
                    WHERE "agt"."chef" = $1 AND "presences"."date" BETWEEN $2::date AND $3::date`, //requete pour faire la soustration entre deus heure differente
                            [1, dateA, dateD],
                            (err, result) => {
                              if (err) {
                                throw err;
                              ...
                              if (result.rows[0]) {
                                const nombreDePresenceEffective =
                                  result.rows[0].temps;
                      ...
                      (SUM(EXTRACT(EPOCH FROM ("presences"."depart" - "presences"."arrive"))::BIGINT) / $1)* 100 AS "pourcentage"
                  FROM
                      "agt"
                  INNER JOIN
                      "presences" ON "presences"."matricule" = "agt"."matricule"
                  WHERE 
                  "agt"."division" =$2  AND "presences"."date" BETWEEN  $3::date AND $4::date AND presences."isdeleted"=$5
                  GROUP BY
                      "presences"."matricule",
                      "agt"."nom",
                      "agt"."prenom",
                  ...
                  ORDER BY
                      "pourcentage" DESC`,
                                  [nombreDeSecondes,'DIRECTION', dateA, dateD,0],  
                                  (err, result) => {
                                    if (err) {
                                      throw err;
                                    }
